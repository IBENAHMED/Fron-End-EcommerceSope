      ...
      <div className=' grid bg-slate-50 pt-7 px-8'>
        <div className='responsive-table container mx-auto bg-slate-50 p-6 rounded-lg shadow-md overflow-auto' style={{maxHeight: "calc(100vh - 105px)"}}>
          ...
          <table className='w-full text-left table-auto border-collapse'>
            <thead className='bg-gray-100 text-gray-600'>
              <tr className='w-full text-xs sm:text-sm border-b border-gray-300'>
        ...
        <div className='py-10 px-3 grid grid-cols-1 md:grid-cols-2 gap-10'>
          <div className='bg-slate-50 shadow-lg rounded-lg p-6'>
            <h1 className='text:xl sm:text-3xl  py-3 font-bold text-gray-800'>Cart Totals</h1>
            <div className='text-xs sm:text-sm flex flex-wrap gap-2 justify-between py-2'>
              <p className='text-gray-600'>Subtotal</p>
              <span className='text-gray-800'>${gettotalPriceProducts()}</span>
            </div>
            <hr className='my-2' />
            <div className='flex text-xs sm:text-sm wrap gap-2 justify-between py-2'>
            ...
            <hr className='my-2' />
            <StripeCheckout stripeKey={`${process.env.NEXT_PUBLIC_PUBLISHABLE_KEY}`} token={makePayment} amount={gettotalPriceProducts() * 100} name='Example Store' description='Purchase' currency='USD' />
          </div>
          <div className='bg-slate-50 shadow-lg rounded-lg p-6'>
