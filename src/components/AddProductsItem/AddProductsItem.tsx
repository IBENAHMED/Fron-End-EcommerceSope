...
const AddProductsItem = () => {
    ...
    let [product, setProduct]: any = useState({name: "", img: "", category: "women", new_price: "", old_price: "", size: []});
    ...
    const handleCheckboxChange = (event: any) => {
      ...
      setSelectedSizes((prevState: any) =>
        checked ? (
          ...
          prevState.filter((size: any) => size !== value)
        )
      );
    };
          ...
          <form>
            ...
            <div className="mb-4">
              <label htmlFor="category" className="block text-sm font-medium text-gray-700">Product Category</label>
                <select onChange={(e: any) => setProduct({ ...product, [e.target.name]: e.target.value })} name='category' id="category" className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-black">
                ...
                <ul className="list-group mt-2">
                  {sizes.map(size => (
                    <li key={size.id} className="list-group-item flex items-center">
                      <input type="checkbox" id={size.id} name={size.id} value={size.id} checked={selectedSizes[size.id]} onChange={handleCheckboxChange} className="form-checkbox h-4 w-4 text-indigo-600 border-gray-300 rounded mr-2"/>
            ...
            <div className="mt-6">
              <button onClick={(e: any) => addProducts(e)} className="w-full flex justify-center mt-4 px-6 py-2 bg-black text-white rounded-lg hover:bg-gray-800">ADD</button>
            </div>
          </form>
        </div>
      </div>
