          ...
          <select onChange={handleClickOption} className='px-3 py-1 rounded text-slate-600'>
            ...
            {isMobile
              ? list.map((e: string, i: number) => {
                  return (
                    <option key={i} className='cursor-pointer p-1 text-gray-600 hover:text-gray-900 text-xs md:text-sm'>{e}</option>
                  )
                })
              : null}
          </select>
        ...
        {!isMobile ? (
          <ul>
            {!isMobile
              ? list.map((e: string, i: number) => {
                  return (
                    <li key={i} onClick={handleClick} className='cursor-pointer p-1 text-gray-600 hover:text-gray-900 text-xs md:text-sm'>{e}</li>
                  )
                })
              : null}
          </ul>
